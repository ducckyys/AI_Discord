...
export const registerMessages = (client: Client) => client.on(Events.MessageCreate, async (message) => {
  ...
  try {
    ...
    const messageText = error instanceof Error && /image|attachment|download|unsupported|large|searxng|internet search|workflow/i.test(error.message) ? error.message : "I could not reach LM Studio right now. Please try again later.";
