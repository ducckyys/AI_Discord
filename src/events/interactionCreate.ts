...
export async function askFromInteraction(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  try { await guildService.ensure(interaction.guildId, interaction.guild?.name ?? "Unknown Guild"); const settings = await settingsService.get(interaction.guildId); const chatImages = await Promise.all(images.map(attachmentToChatImage)); const answer = await aiService.askDetailed({ guildId: interaction.guildId, userId: interaction.user.id, channelId: interaction.channelId, question: parsed.data, maxContextMessages: settings.maxContextMessages, model: settings.model, images: chatImages }); const chunks = splitMessage(answer.content); await interaction.editReply({ content: chunks[0] ?? "Done.", files: toDiscordAttachments(answer.files) }); for (const chunk of chunks.slice(1)) await interaction.followUp({ content: chunk, allowedMentions: { repliedUser: false } }); }
  catch (error) { logger.error({ err: error }, "AI request failed"); const message = error instanceof Error && /image|attachment|download|unsupported|large|searxng|internet search|comfyui|workflow/i.test(error.message) ? error.message : "I could not reach LM Studio. Please ensure it is running and a model is loaded."; await interaction.editReply({ embeds: [errorEmbed(message)] }); }
}
