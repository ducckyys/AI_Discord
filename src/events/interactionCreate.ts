...
export async function askFromInteraction(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  try { const settings = await settingsService.get(interaction.guildId); const answer = await aiService.ask({ guildId: interaction.guildId, userId: interaction.user.id, channelId: interaction.channelId, question: parsed.data, maxContextMessages: settings.maxContextMessages, model: settings.model }); await interaction.editReply(answer); }
